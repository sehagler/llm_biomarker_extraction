...
#SBATCH --job-name=biomarkers_prompts_pr
#SBATCH --output=out/biomarkers_prompts_pr_%j.out
...
PICKLE_PR="/home/exacloud/gscratch/OCTRI_AI_NLP/haglers/pkl/breast_cancer_biomarkers_pr.pkl"
PORT=8080
...
srun -c 20 --mem-per-cpu=10G --gres disk:512,gpu:v100:4 --exclusive python3 $SCRIPT --cuda --data $DATA --keywords $KEYWORDS --layout $LAYOUT --mode $MODE_PR --model $MODEL --pickle $PICKLE_PR --port $PORT --tokenizer $TOKENIZER